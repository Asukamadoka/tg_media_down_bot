...
3. **手动指定端点**（M7.2 C）：经代理拿到的 Telegram 配置里，DC4 的 IPv4 媒体端点是
   `149.154.167.43`、DC2 的是 `149.154.167.222`，这两个从 NAS 都直连不通；能直连的
   `149.154.166.111/.110/.120` 只在直连拿到的配置里。所以在 `.env` 里写上：
   ```
   TG_DIRECT_ENDPOINTS=4=149.154.166.111:443,4=149.154.166.110:443,4=149.154.166.120:443
   ```
   这些端点排在最前面，只对 v2 生效。**另外两个 `.110`、`.120` 也要加 DIRECT 规则**
   （写法同上，放在 Telegram 段之前），否则它们会走 `TG` 组。
   写错 DC 的端点会导入授权失败，这个 DC 冷却 24 小时，日志里会写出是哪个端点。
4. `.env` 里设 `TG_DIRECT_MEDIA=v2`，`docker compose up -d bot`。日志里每个文件一行，
